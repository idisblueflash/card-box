...
2. **抽取关键信息**：在技能中完成标题命名、1-2 句摘要与标签决策（默认 `fleeting`）。摘要应在复述用户原话的基础上做“轻度压缩”，不得引入新场景、新指标或未被用户提及的推断。  
...
- **忠实压缩**：摘要必须 100% 建立在用户输入的原意之上，只允许合并句子、删除重复或语气词；不得补充测试步骤、时间节点、行动计划等新增内容。若用户强调“反思/复盘”等语义，请保留这些关键词。  
