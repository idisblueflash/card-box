...
def test_quote_skill_trigger_from_fixture():
    ...
    with manifest_path.open(encoding="utf-8") as fh:
        for line in fh:
            ...
            expectations[case["log_file"]] = case.get("expected_trigger", True)

    json_files = sorted((fixtures_dir / name) for name in expectations)
    ...
    for path in json_files:
        ...
        expected = expectations.get(path.name, True)
        assert trigger == expected, f"{path.name}: trigger mismatch (expected {expected}, got {trigger})."
